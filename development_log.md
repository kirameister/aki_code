...
こうすることで、送り仮名なども含めた内容を入力して、辞書の manipulation を行うことなく効率的な入力ができるかもしれない (現状では単なるアイデア)。

## 2023-11-25

久しぶりに開発 (と言ってよいのか?) をしていて思ったのだが、覚えるのがかなり大変ということを再度確認した。9x9 のベースでこんな状態なのだから、30x30 ですべて覚えるのは絶望的だろう。

…というわけで少し考えてみたのだけれど、

* 30x30=900 も、覚えられないのであれば意味がないので、このレベルの coverage は諦める。
* そもそも、意味や熟語をベースに個々の漢字の配置を覚えるのに無理があったのではないか。
* 漢字を書くときには、意味を考えるとこもあるだろうが、そもそも頭に思い浮かぶのはその表記ではないだろうか。

…というわけで、一旦これまで考えていたことをご破産にして、以下のような方針で配列を考えてみようと思う:

* 「部首」とは少し違うが、個々の漢字の「部分」をストロークのキーとする。
* 「部首/部分」には、主にそれだけでも漢字として存在しているか、すぐに連想できるものを使う。
* 「部分」のキーの二度押しで、その漢字が入力 (出力?) されるようにする。
* 部分同士のキーを押すことで組み合わされた漢字が入力される。
* 順序には基本的に向きを無視して良いと考えている。これにより入力できる漢字の可能性が半分程度になるはずなのだが、これでも 450-500 ぐらいはできるはず…なのだが、想像するにこの方法だけではすべて埋めることはできないかも。
* この考え方で埋められるところを埋めていき、あとはユーザーが指定できるようにするという割り切った考え方でも良いかも知れない。

とりあえず手を動かしてみるか。今までの成果は (ログも含めて) development_gravesone とでも名付けたブランチにおいておくことにしよう。
